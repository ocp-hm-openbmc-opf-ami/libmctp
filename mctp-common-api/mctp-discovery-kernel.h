...
#ifndef __MCTP_KERNEL_DISCOVERY_H__
#define __MCTP_KERNEL_DISCOVERY_H__
...
uint8_t mctp_kernel_get_kernel_bus(int eid);
uint8_t mctp_kernel_get_kernel_addr(int eid);

mctp_ret_codes_t mctp_kernel_get_mctp_ver_support_request(int sock_fd,
						       uint8_t eid);
...
mctp_ret_codes_t mctp_kernel_alloc_eid_send_request(int sock_fd,
						 ...
						 uint8_t eid_count,
						 uint8_t eid_start);
int mctp_kernel_alloc_eid_get_response(uint8_t *mctp_resp_msg,
				    size_t resp_msg_len);
