...
static void test_encode_get_msg_type_support_req()
{
	encode_rc ret;
	uint8_t expected_instance_id = 0x01;
	...
	ret = mctp_encode_get_msg_type_support_req(
...
static void test_negative_encode_get_msg_type_support_req()
{
	encode_rc ret;
	uint8_t expected_instance_id = 0x01;
	uint8_t rq_d_inst = expected_instance_id | MCTP_CTRL_HDR_FLAG_REQUEST;
	...
	ret = mctp_encode_get_msg_type_support_req(
		request, sizeof(struct mctp_ctrl_cmd_get_msg_type_support),
		rq_d_inst);
	assert(ret == ENCODE_INPUT_ERROR);
	struct mctp_msg request1;
	ret = mctp_encode_get_msg_type_support_req(&request1, 0, rq_d_inst);
	assert(ret == ENCODE_GENERIC_ERROR);
}
...
int main(int argc, char *argv[])
{
	...
	test_encode_get_msg_type_support_req();
	...
	test_negative_encode_get_msg_type_support_req();
