...
static void test_decode_get_msg_type_support_resp()
{
	decode_rc ret;
	...
	uint8_t rq_d_inst = expected_instance_id | MCTP_CTRL_HDR_FLAG_REQUEST;
	response.ctrl_hdr.rq_dgram_inst = rq_d_inst;
	response.ctrl_hdr.command_code = MCTP_CTRL_CMD_GET_MESSAGE_TYPE_SUPPORT;
	response.msg_type_count = 1;
	response.completion_code = MCTP_CTRL_CC_SUCCESS;
	uint8_t msg_type_count;
	uint8_t completion_code;
	struct mctp_msg *resp = (struct mctp_msg *)(&response);

	ret = mctp_decode_get_msg_type_support_resp(
		resp, sizeof(struct mctp_ctrl_resp_get_msg_type_support),
		&ctrl_hdr, &completion_code, &msg_type_count);
	assert(ret == DECODE_SUCCESS);
...
static void test_negative_decode_get_msg_type_support_resp()
{
	decode_rc ret;
	struct mctp_ctrl_msg_hdr ctrl_hdr;
	struct mctp_ctrl_resp_get_msg_type_support response;
	...
	uint8_t completion_code;
	ret = mctp_decode_get_msg_type_support_resp(
		NULL, sizeof(struct mctp_ctrl_resp_get_msg_type_support),
		&ctrl_hdr, &completion_code, &msg_type_count);
	assert(ret == DECODE_INPUT_ERROR);
	ret = mctp_decode_get_msg_type_support_resp(
		resp, sizeof(struct mctp_ctrl_resp_get_msg_type_support), NULL,
		&completion_code, &msg_type_count);
	assert(ret == DECODE_INPUT_ERROR);
	ret = mctp_decode_get_msg_type_support_resp(
		resp, sizeof(struct mctp_ctrl_resp_get_msg_type_support),
		&ctrl_hdr, NULL, &msg_type_count);
	assert(ret == DECODE_INPUT_ERROR);
	ret = mctp_decode_get_msg_type_support_resp(
		resp, sizeof(struct mctp_ctrl_resp_get_msg_type_support),
		&ctrl_hdr, &completion_code, NULL);
	...
	ret = mctp_decode_get_msg_type_support_resp(
		resp, 0, &ctrl_hdr, &completion_code, &msg_type_count);
	assert(ret == DECODE_GENERIC_ERROR);
	response.completion_code = MCTP_CTRL_CC_ERROR;
	ret = mctp_decode_get_msg_type_support_resp(
		resp, sizeof(struct mctp_ctrl_resp_get_msg_type_support),
...
int main(int argc, char *argv[])
{
	...
	test_decode_get_msg_type_support_resp();
	...
	test_negative_decode_get_msg_type_support_resp();
