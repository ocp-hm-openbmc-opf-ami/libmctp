...
static void test_decode_discovery_notify_resp()
{
	decode_rc ret;
	...
	uint8_t rq_d_inst = expected_instance_id | MCTP_CTRL_HDR_FLAG_REQUEST;
	response.ctrl_hdr.rq_dgram_inst = rq_d_inst;
	response.ctrl_hdr.command_code = MCTP_CTRL_CMD_DISCOVERY_NOTIFY;
	response.completion_code = MCTP_CTRL_CC_SUCCESS;
	uint8_t completion_code;
	struct mctp_msg *resp = (struct mctp_msg *)(&response);

	ret = mctp_decode_discovery_notify_resp(
		resp, sizeof(struct mctp_ctrl_resp_discovery_notify), &ctrl_hdr,
		&completion_code);
	assert(ret == DECODE_SUCCESS);
...
static void test_negative_decode_discovery_notify_resp()
{
	decode_rc ret;
	struct mctp_ctrl_msg_hdr ctrl_hdr;
	struct mctp_ctrl_resp_discovery_notify response;
	...
	uint8_t completion_code;
	ret = mctp_decode_discovery_notify_resp(
		NULL, sizeof(struct mctp_ctrl_resp_discovery_notify), &ctrl_hdr,
		&completion_code);
	assert(ret == DECODE_INPUT_ERROR);
	ret = mctp_decode_discovery_notify_resp(
		resp, sizeof(struct mctp_ctrl_resp_discovery_notify), NULL,
		&completion_code);
	assert(ret == DECODE_INPUT_ERROR);
	ret = mctp_decode_discovery_notify_resp(
		resp, sizeof(struct mctp_ctrl_resp_discovery_notify), &ctrl_hdr,
	...
	ret = mctp_decode_discovery_notify_resp(resp, 0, &ctrl_hdr,
						&completion_code);
	assert(ret == DECODE_GENERIC_ERROR);
	response.completion_code = MCTP_CTRL_CC_ERROR;
	ret = mctp_decode_discovery_notify_resp(
...
int main(int argc, char *argv[])
{
	...
	test_decode_discovery_notify_resp();
	...
	test_negative_decode_discovery_notify_resp();
