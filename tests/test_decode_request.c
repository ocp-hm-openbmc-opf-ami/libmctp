...
static void test_decode_get_msg_type_support_req()
{
	decode_rc ret;
	uint8_t expected_instance_id = 0x01;
	struct mctp_ctrl_cmd_get_msg_type_support request;
	struct mctp_ctrl_msg_hdr ctrl_hdr;
	request.ctrl_msg_hdr.ic_msg_type = MCTP_CTRL_HDR_MSG_TYPE;
	uint8_t rq_d_inst = expected_instance_id | MCTP_CTRL_HDR_FLAG_REQUEST;
	request.ctrl_msg_hdr.rq_dgram_inst = rq_d_inst;
	...
	ret = mctp_decode_get_msg_type_support_req(
		req, sizeof(struct mctp_ctrl_cmd_get_msg_type_support),
		&ctrl_hdr);
	assert(ret == DECODE_SUCCESS);
	assert(ctrl_hdr.command_code == request.ctrl_msg_hdr.command_code);
	assert(ctrl_hdr.rq_dgram_inst == request.ctrl_msg_hdr.rq_dgram_inst);
...
static void test_negative_decode_get_msg_type_support_req()
{
	decode_rc ret;
	struct mctp_ctrl_cmd_get_msg_type_support request;
	struct mctp_ctrl_msg_hdr ctrl_hdr;
	...
	ret = mctp_decode_get_msg_type_support_req(
		NULL, sizeof(struct mctp_ctrl_cmd_get_msg_type_support),
		&ctrl_hdr);
	assert(ret == DECODE_INPUT_ERROR);
	ret = mctp_decode_get_msg_type_support_req(
		req, sizeof(struct mctp_ctrl_cmd_get_msg_type_support), NULL);
	assert(ret == DECODE_INPUT_ERROR);

	ret = mctp_decode_get_msg_type_support_req(req, 0, &ctrl_hdr);
	assert(ret == DECODE_GENERIC_ERROR);
	request.ctrl_msg_hdr.command_code = MCTP_CTRL_CMD_RESOLVE_ENDPOINT_ID;
	ret = mctp_decode_get_msg_type_support_req(
		req, sizeof(struct mctp_ctrl_cmd_get_msg_type_support),
...
int main(int argc, char *argv[])
{
	...
	test_decode_get_msg_type_support_req();
	...
	test_negative_decode_get_msg_type_support_req();
